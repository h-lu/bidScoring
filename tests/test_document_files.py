    ...
    def test_document_files_foreign_key(self):
        ...
        with get_test_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                assert len(version_fk) > 0, "version_id should have FK to document_versions"
    ...
    def test_chunks_has_embedding_status(self):
        ...
        with get_test_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                assert "pending" in default_val or result.get("column_default") == "'pending'::text", \
    ...
    def test_chunks_embedding_status_index(self):
        ...
        with get_test_conn() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                ...
                assert result is not None, "Should have idx_chunks_embedding_status index"
...
class TestFileRegistryOperations:
    ...
    def test_version(self):
        ...
        with get_test_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("INSERT INTO projects (project_id, name) VALUES (%s, %s) ON CONFLICT DO NOTHING",
                           (project_id, "test-project"))
                cur.execute("INSERT INTO documents (doc_id, project_id, title, source_type) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING",
                           (doc_id, project_id, "test-doc", "mineru"))
                cur.execute("INSERT INTO document_versions (version_id, doc_id, source_uri, parser_version, status) VALUES (%s, %s, %s, %s, %s) ON CONFLICT DO NOTHING",
        ...
        with get_test_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("DELETE FROM document_files WHERE version_id = %s", (version_id,))
    ...
    def test_register_file(self, test_version):
        ...
        with get_test_conn() as conn:
            ...
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT * FROM document_files WHERE file_id = %s", (file_id,))
                record = cur.fetchone()
    ...
    def test_get_files_by_version(self, test_version):
        ...
        with get_test_conn() as conn:
            ...
            registry.register_file(
                version_id=test_version, file_type="original_pdf",
                file_path="bids/test/v1/files/original/test.pdf",
                file_name="test.pdf", file_size=1000,
                content_type="application/pdf", etag="1"
            )
            registry.register_file(
                version_id=test_version, file_type="markdown",
                file_path="bids/test/v1/files/parsed/full.md",
                file_name="full.md", file_size=500,
                content_type="text/markdown", etag="2"
            )
    ...
    def test_register_file_duplicate_path(self, test_version):
        ...
        with get_test_conn() as conn:
            ...
            registry.register_file(
                version_id=test_version, file_type="original_pdf",
                file_path=file_path, file_name="test.pdf",
