...
class HiChunkBuilder:
    ...
    def _should_start_new_paragraph(
        self, prev_item: Optional[dict], curr_item: dict
    ) -> bool:
        """判断是否应该开始新段落
    ...
    def _create_paragraph_nodes(
        self, leaf_nodes: list[HiChunkNode], content_list: list[dict]
    ) -> list[HiChunkNode]:
        ...
        for i, leaf in enumerate(leaf_nodes):
            ...
            curr_content = (
                content_list[source_idx] if source_idx < len(content_list) else None
            )
    ...
    def _create_paragraph_from_leaves(self, leaves: list[HiChunkNode]) -> HiChunkNode:
        ...
        bboxes = [
            leaf.metadata.get("bbox", [])
            for leaf in leaves
            if leaf.metadata.get("bbox")
        ]
        element_types = list(
            set(leaf.metadata.get("element_type", "") for leaf in leaves)
        )
    ...
    def _create_section_nodes(
        self, paragraph_nodes: list[HiChunkNode], content_list: list[dict]
    ) -> list[HiChunkNode]:
        ...
        if not heading_info:
            section_node = HiChunkNode(
                ...
                metadata={
                    "section_title": "默认章节",
                    "heading_level": 0,
                    "page_idx": paragraph_nodes[0].metadata.get("start_page", 0)
                    if paragraph_nodes
                    else 0,
                },
            )
    ...
    def _create_root_node(
        self, section_nodes: list[HiChunkNode], document_title: str
    ) -> HiChunkNode:
    ...
    def build_hierarchy(
        self, content_list: list[dict], document_title: str = "untitled"
    ) -> list[HiChunkNode]:
        """构建完整的层次结构
