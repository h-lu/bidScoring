...
class SmartChunkMerger:
    ...
    def merge_chunks(
        self, raw_chunks: List[Dict[str, Any]], section_heading: str = ""
    ) -> List[ProcessedChunk]:
        ...
        sorted_chunks = sorted(
            raw_chunks, key=lambda c: (c.get("page_idx", 0), c.get("chunk_index", 0))
        )
    ...
    def _flush_buffer(self, buffer: List[Dict[str, Any]]) -> ProcessedChunk:
        ...
        # Support both 'text_raw' (MinerU) and 'content' (ParagraphMerger) keys
        content = "\n".join(
            c.get("text_raw", c.get("content", "")).strip() for c in buffer
        )
        ...
        return ProcessedChunk(
            ...
            page_end=page_end,
            metadata={"merged_count": len(buffer), "original_char_count": len(content)},
        )
            ...
            if chunk.char_count >= self.min_size:
                if pending_small:
                    ...
                    chunk = ProcessedChunk(
                        ...
                        char_count=min(
                            len(merged_embedding_content), self.max_size * 2
                        ),
                        source_chunk_ids=pending_small.source_chunk_ids
        ...
        if pending_small:
            if result:
                ...
                result[-1] = ProcessedChunk(
                    ...
                    + pending_small.source_chunk_ids,
...
class SectionChunkBuilder:
    ...
    def build_sections_with_chunks(
        self, paragraphs: List[Dict[str, Any]], document_title: str = ""
        ...
        def finalize_section():
            ...
            processed_chunks = self.merger.merge_chunks(
                state["current_chunks_raw"], state["current_section"].heading
            )
        ...
        for para in paragraphs:
            if para.get("is_heading"):
                ...
                state["current_all_sources"] = para.get(
                    "source_chunk_ids", []
                ) or para.get("source_chunks", [])
            else:
                # Regular paragraph
                ...
                # Accumulate sources
                para_sources = para.get("source_chunk_ids", []) or para.get(
                    "source_chunks", []
                )
...
class ParagraphMerger:
    ...
    def __init__(
        self, min_length: int = 80, max_length: int = 500, short_threshold: int = 20
    ):
    ...
    def merge(self, chunks: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        sorted_chunks = sorted(
            chunks, key=lambda c: (c.get("page_idx", 0), c.get("chunk_index", 0))
        )
        ...
        for chunk in sorted_chunks:
            ...
            if buffer and self._sentence_end_pattern.search(
                buffer[-1].get("text_raw", "")
            ):
                paragraphs.append(self._create_paragraph(buffer))
                buffer = []
                current_length = 0
            ...
            if not buffer:
                ...
                len(chunk_text) < self.min_length
                and (current_length + len(chunk_text)) <= self.max_length
            ):
                buffer.append(chunk)
            else:
...
def create_small_to_big_sections(
    ...
    paragraph_merger = ParagraphMerger(
        min_length=min_chunk_size, max_length=max_chunk_size
    )
